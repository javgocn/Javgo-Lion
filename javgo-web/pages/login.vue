          ...
          <div v-if="!(userInfo)" :class="{login_form: true, rotate: tab == 2}">
            ...
            <div v-if="tabp == 1" class="form_body">
              <form action="" @submit="loginSubmit">
                ...
                <div>
                  <input v-model="imageVerification" type="text" placeholder="请输入验证码" style="width: 200px"/>
                  <img :src="imgCode" class="img_code" @click="getImgCode" style="width: 90px;height: 40px;float: right;margin-top: 20px;margin-right: 20px"/>
                </div>
...
import { register, userLogin, getCodeImg } from '@/api/login.js'
export default {
  components: {
    YHeader,
    YButton
  ...
  methods: {
    getImgCode() {
      getCodeImg().then((res) => {
        ...
        this.imageVerificationToken = res.verToken
    ...
    loginSubmit(e) {
      ...
      this.loginObj.imageVerification = this.imageVerification
      this.loginObj.imageVerificationToken = this.imageVerificationToken
