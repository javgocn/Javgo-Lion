      ...
      <div class="i_zone">
        <div class="zone_header">
          <div class="big_text">
            <span class="col_block" />
            讲师专区
            <span class="small_text">优质教师</span>
          </div>
        </div>
        <div class="zone_body">
          <ul class="clearfix course_list">
            <li v-for="(item,index) in zoneTeacher" :key="index" style="margin-top: 15px">
                <div class="img_box" style="text-align: center">
                  <img :src="item.imageUrl" alt="" style="width: 128px;height: 128px;border-radius: 50%" >
                </div>
                <h2 style="text-align: center;font-size: 24px">{{ item.name }}</h2>
                <div style="text-align: center" v-html="item.introduce" ></div>
            </li>
          </ul>
        </div>
      </div>
...
import { carouselList, categoryList, zoneList, zoneTeacherFrontList } from '~/api/main.js'
...
export default {
  ...
  async asyncData(context) {
    try {
      ...
      // 讲师
      dataObj.zoneTeacher = await zoneTeacherFrontList()
      return dataObj
    } catch (e) {
